...
def gradient(f, dim, spacing, order=4):
    ...
    coefficients = {
        2: ([-1/2, 1/2], [-1, 1]),
        4: ([-1/12, 8/12, -8/12, 1/12], [-2, -1, 1, 2])
    }
    coeffs, shifts = coefficients[order]
    coeffs = torch.tensor(coeffs, dtype=f.dtype, device=f.device)
    gradients = []
    for d, h in zip(dim, spacing):
        gradients.append(torch.einsum("i,i...", coeffs, torch.stack([torch.roll(f, shift, dims=d) for shift in shifts])) / h)
